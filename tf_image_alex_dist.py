...
def restoreParamsFromModels(num_model):
  ...
  for i in range(1,num_model+1):
    ...
    w1_1 = graph.get_tensor_by_name("W1_1:0")
    w1_2 = graph.get_tensor_by_name("W1_2:0")
    w2_1 = graph.get_tensor_by_name("W2_1:0")
    w2_2 = graph.get_tensor_by_name("W2_2:0")
    print "small model %d is restored." % i

    filter_W1_1 = sess.run(w1_1,feed_dict={X: x, Y_: y, keep_prob: 1.0})
    filter_W1_2 = sess.run(w1_2,feed_dict={X: x, Y_: y, keep_prob: 1.0})
    filter_W2_1 = sess.run(w2_1,feed_dict={X: x, Y_: y, keep_prob: 1.0})
    filter_W2_2 = sess.run(w2_2,feed_dict={X: x, Y_: y, keep_prob: 1.0})

    if i == 1:
      pre_trained_W1_1 = filter_W1_1
      pre_trained_W1_2 = filter_W1_2
      pre_trained_W2_1 = filter_W2_1
      pre_trained_W2_2 = filter_W2_2
    else:
      pre_trained_W1_1 = np.concatenate((pre_trained_W1_1,filter_W1_1),axis=-1)
      pre_trained_W1_2 = np.concatenate((pre_trained_W1_2,filter_W1_2),axis=-1)
      pre_trained_W2_1 = np.concatenate((pre_trained_W2_1,filter_W2_1),axis=2)
      pre_trained_W2_2 = np.concatenate((pre_trained_W2_2,filter_W2_2),axis=2)


    print pre_trained_W1_1.shape
    print pre_trained_W1_2.shape
    print pre_trained_W2_1.shape
    print pre_trained_W2_2.shape


  tf.reset_default_graph()

  W1_1 = tf.Variable(pre_trained_W1_1, name="W1_1")
  W1_2 = tf.Variable(pre_trained_W1_2, name="W1_2")
  W2_1 = tf.Variable(pre_trained_W2_1, name="W2_1")
  W2_2 = tf.Variable(pre_trained_W2_2, name="W2_2")

  wd1_1 = tf.multiply(tf.nn.l2_loss(W1_1), 5e-4, name='weight_loss')
  wd1_2 = tf.multiply(tf.nn.l2_loss(W1_2), 5e-4, name='weight_loss')
  wd2_1 = tf.multiply(tf.nn.l2_loss(W2_1), 5e-4, name='weight_loss')
  wd2_2 = tf.multiply(tf.nn.l2_loss(W2_2), 5e-4, name='weight_loss')
  tf.add_to_collection('losses', wd1_1)
  tf.add_to_collection('losses', wd1_2)
  tf.add_to_collection('losses', wd2_1)
  tf.add_to_collection('losses', wd2_2)
...
if __name__ == '__main__':
  ...
  W1_1, W1_2, W2_1, W2_2 = restoreParamsFromModels(4)
